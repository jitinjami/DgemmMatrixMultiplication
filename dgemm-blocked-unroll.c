const char *dgemm_desc = "Unrolled blocked dgemm.";
...
static void do_block_unroll(int n, int dim_A, int dim_B, int dim_C, double *A, double *B, double *C) {
    ...
    for (int i = 0; i < n; i += BLOCK_SIZE) {
        for (int j = 0; j < n; j += BLOCK_SIZE) {
            for (int k = 0; k < n; k += BLOCK_SIZE) {
                ...
                do_block_unroll(n, dim_A, dim_B, dim_C, A + i + k * n, B + k + j*n, C + i + j*n);
            }
        }
    }
    free(A_t);
